package br.com.gilmariosoftware.masterdicas.servico.cliente;

import javax.xml.ws.WebFault;

/**
 * This class was generated by the JAX-WS RI. JAX-WS RI 2.2.8 Generated source
 * version: 2.2
 *
 */
@WebFault(name = "Fault", targetNamespace = "http://mestredasdicas-gilserver.openshift.com")
public class FaultException
        extends Exception {

    /**
     * Java type that goes as soapenv:Fault detail element.
     *
     */
    private final String faultInfo;

    /**
     *
     * @param faultInfo
     * @param message
     */
    public FaultException(String message, String faultInfo) {
        super(message);
        this.faultInfo = faultInfo;
    }

    /**
     *
     * @param faultInfo
     * @param cause
     * @param message
     */
    public FaultException(String message, String faultInfo, Throwable cause) {
        super(message, cause);
        this.faultInfo = faultInfo;
    }

    /**
     *
     * @return returns fault bean: java.lang.String
     */
    public String getFaultInfo() {
        return faultInfo;
    }

}
